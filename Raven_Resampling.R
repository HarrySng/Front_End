...
gauge_data <- readRDS(paste(raven_dir,'raw_klk_tmp_pr.rds', sep=""))[[1]] # Read tmp, pr data
# This is a list of 7 datasets (1 Guage, 1 AHCCD, 5 gridded products)

# Resample rows of gauge data 100 times, generate discharge

res_list <- list(run_raven(gauge_data))
for (i in 2:999) {
  print(i)
  res_list[[i]] <- run_raven(gauge_data[sample(nrow(gauge_data)),])
}
saveRDS(res_list,'D:/PAPER2/Raven_biv/resampled_results.rds',compress = F)